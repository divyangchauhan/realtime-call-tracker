 ...
 *   transitions - the Postgres row stays at its initial QUEUED status until the
 *   periodic flush reconciles it.  Each non-terminal transition also marks the
 *   call ID dirty (CallStateStore.markDirty) so CallFlushService knows to
 ...
 *   `call:events` channel so the WebSocket gateway can fan it out to
 *   subscribed clients.
 *
...
export class CallProgressionService implements OnModuleDestroy {
  ...
  private async applyTransition(state: CallState, newStatus: CallStatus): Promise<void> {
    ...
    // Dirty-mark for the periodic flush
    ...
    // call:events channel.  The WebSocket gateway subscribes and forwards
