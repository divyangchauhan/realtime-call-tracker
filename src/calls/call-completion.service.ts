 ...
 *         Postgres durably. The periodic flush reconciles any intermediate states.
 *
 *  2. **BullMQ recording dispatch (best-effort):**
 *     Enqueue a recording upload job onto the 'recording' BullMQ queue.
 *     The recording worker consumes this job, uploads the recording to S3, and
 *     writes the resulting URL back (recording_url in Postgres + Redis).
 *
 ...
 *                        reconciled by the periodic flush pass.
...
export class CallCompletionService {
  ...
  async complete(state: CallState): Promise<void> {
    ...
    try {
      ...
      // call we couldn't durably mark complete - the periodic flush will reconcile.
      this.logger.warn(
