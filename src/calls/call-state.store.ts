 ...
 *
 * CONTRACT - several modules depend on this shape:
 * - The rate limiter reads `apiKeyId` + `status` for rate-limit accounting.
 * - The progression engine reads/writes `status` when driving the state machine.
 * - The WebSocket gateway publishes state-change events keyed by `id`.
 * - The recording worker writes `recordingUrl` after the upload completes.
 *
...
export interface CallState {
  ...
  /** S3 URL written by the recording worker. Null until then. */
 ...
 * CallFlushService periodically drains it, writing each ID's latest
 ...
 * - The WebSocket gateway fans out state-change events via pub/sub;
...
export class CallStateStore {
   ...
   *
   * Called by the BullMQ recording worker after a successful S3 upload.
   ...
   * transition so CallFlushService knows which IDs to reconcile.
   *
