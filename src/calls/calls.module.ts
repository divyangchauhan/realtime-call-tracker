 ...
 *
 * CallProgressionService is registered here so it shares the same
 * DI scope as CallsService and can be injected into it without a circular dep.
 *
 * CallCompletionService handles the two durable COMPLETED side-effects:
 ...
 *
 * CallStateStore is exported so WebsocketModule can inject it for the
 * initial snapshot read on WebSocket connect.  RedisModule is @Global() so
 * WebsocketModule does not need to import it explicitly.
 *
 * CallFlushService periodically reconciles the Redis write-behind
...
@Module({
  ...
  providers: [
    ...
    // CallFlushService: periodic write-behind reconciler, API process only.
    CallFlushService,
  ],
