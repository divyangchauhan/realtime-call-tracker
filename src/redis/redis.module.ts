 ...
 *    WebSocket gateway to SUBSCRIBE to call:events.  A connection in
 ...
 *
 * BullMQ (via @nestjs/bullmq) opens a THIRD separate connection with
    ...
    // COMMAND connection
    {
      ...
      useFactory: (config: ConfigService<Configuration, true>): Redis => {
        ...
        return new Redis({
          ...
          // is down, and the Lua rate-limit gate should reject quickly
          ...
          // belongs ONLY on the SEPARATE BullMQ connection (created
